...
public class SPSiddhiAppCreator extends AbstractSiddhiAppCreator {
    ...
    @Override
    public List<SiddhiQuery> createApps(String siddhiAppName, SiddhiQueryGroup queryGroup) {
     ...
     * @param queryList     Contains the query of the current execution group replicated to the parallelism of the group.
            ...
            for (PublishingStrategyDataHolder holder : outputStream.getPublishingStrategyList()) {
                ...
                if (holder.getStrategy() == TransportStrategy.FIELD_GROUPING) {
                    ...
                    for (int i = 0; i < holder.getParallelism(); i++) {
                        ...
                        destinationMap.put(ResourceManagerConstants.PARTITION_TOPIC, sinkValuesMap.get(ResourceManagerConstants.MB_DESTINATION)
                                + "_" + String.valueOf(i));
     ...
     *
     * Assign the jms transport headers for each instance siddhi applications input streams for a given execution group
     */
                ...
                if (subscriptionStrategy.getStrategy() == TransportStrategy.FIELD_GROUPING) {
                    ...
                    for (int i = 0; i < queryList.size(); i++) {
                        ...
                        for (int topicCount : partitionNumbers){
                            String topicName = siddhiAppName + "_" + inputStream.getStreamName() + "_" +
                                    inputStream.getSubscriptionStrategy().getPartitionKey() + "_" + Integer.toString(topicCount);
                            sourceValuesMap.put(ResourceManagerConstants.MB_DESTINATION,topicName);


                            String sourceQuery = getUpdatedQuery(ResourceManagerConstants.DEFAULT_MB_TOPIC_SOURCE_TEMPLATE,sourceValuesMap);
                    ...
                    if (rrTrackerMap.get(inputStream.getStreamName()) != null){
                        ...
                        queueName = siddhiAppName + "_" + inputStream.getStreamName() + "_" +Integer.toString(queueCount);
                        queueCount += 1;
